...
def web_search_node(state):
    ...
    try:
        ...
        for r in response["results"]:
            # Tavily'den gelen 'url' bilgisini 'source' metadata'sına koyuyoruz
            doc = Document(page_content=r["content"], metadata={"source": r["url"]})
...
def generate_node(state):
    ...
    # --- DÜZELTİLMİŞ KAYNAK MANTIĞI ---
    context_list = []
    for d in documents:
        # Kaynak bilgisini al, yoksa "Unknown" de
        raw_source = d.metadata.get("source", "Unknown")

        # Eğer kaynak bir web linki ise (http ile başlıyorsa) dokunma, olduğu gibi kullan
        if raw_source.startswith("http") or raw_source.startswith("www"):
            display_source = raw_source
        else:
            # Dosya ise sadece ismini al (örn: /home/data.json -> data.json)
            display_source = os.path.basename(raw_source)
...
def save_graph_image():
    try:
        # Grafiği Mermaid PNG formatına çevirir (Binary veri döner)
        ...
        print(f"❌ Grafik oluşturulamadı. Hata: {e}")
        # Not: Bazen ekstra kütüphane isteyebilir.
...
if __name__ == "__main__":
    ...
    print("\n--- TEST 2: False Correlation (Golden Dome vs Super Bowl) ---")
    test_reranked_rag_query(
        user_query="Exactly how did the architectural design of the 'Golden Dome' at the University of Notre Dame serve as the specific inspiration for the 'Golden Anniversary' theme of Super Bowl 50?",
    )
    test_reranked_rag_query(
